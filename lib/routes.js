...
module.exports = [
	...
	},
	{
		...
		config: {
			...
			description: 'Get',
			tags: ['api'],
			notes: ['Get previously trained cost history'],
		}
	},
	{
		method: 'GET',
		path: '/v1/model/error-rate',
		config: {
			handler: async (request, h) => {
				try {
					//const equals = request.params.a + request.params.b
			...
			description: 'Get',
			tags: ['api'],
			notes: ['Get previously trained error rate as a percentage'],
		}
	},
	{
		method: 'POST',
		...
		config: {
			handler: async (request, h) => {
				try {
					return h.response(ml.train(
						request.payload.learningRate,
						request.payload.iterations,
						request.payload.batchSize));
				} catch (err) {
					return h.response(err.message).code(500);
				}
			},
			description: 'Train',
			tags: ['api'],
			notes: ['Runs the training and creates model weights'],
			validate: {
				payload: {
					learningRate: Joi.number()
						.required()
						.default(1)
						.description('the learning rate to start with'),
